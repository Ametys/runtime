...
Ext.define('Ametys.plugins.admin.Users', {
	singleton: true,
	...
	{
		// The users
		Ext.define('Ametys.plugins.admin.Users.Users', {
		...
		this._listView = new Ext.grid.Panel({
		    ...
		    store : Ext.create('Ext.data.Store', {
				model: 'Ametys.plugins.admin.Users.Users',
		        data: { users: []},
		    ...
		    columns: [
				        {header: "<i18n:text i18n:key='PLUGINS_CORE_USERS_COL_NAME' i18n:catalogue='plugin.core'/>", flex: 0.4, menuDisabled : true, sortable: true, dataIndex: 'display'},
				        {header: "<i18n:text i18n:key='PLUGINS_CORE_USERS_COL_EMAIL' i18n:catalogue='plugin.core'/>", flex: 0.6, menuDisabled : true, sortable: true, dataIndex: 'email'}
		    ],
	...
	{
		this._search = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_USERS_SEARCH' i18n:catalogue='plugin.core'/>"});
		...
		var ct = new Ext.Container({
			...
			items: [
				new Ext.form.field.Text({
				    ...
				    value: '<i18n:text i18n:key="PLUGINS_CORE_USERS_SEARCH_CRITERIA" i18n:catalogue="plugin.core"/>',
				    onFocus : function () {if (this.getValue() == '<i18n:text i18n:key="PLUGINS_CORE_USERS_SEARCH_CRITERIA" i18n:catalogue="plugin.core"/>') this.setValue(''); },
				    onBlur : function () {if (this.getValue() == '') this.setValue('<i18n:text i18n:key="PLUGINS_CORE_USERS_SEARCH_CRITERIA" i18n:catalogue="plugin.core"/>'); }
				}),

				new Ext.button.Button ({
		            icon: Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/search_16.png',
		            tooltip: "<i18n:text i18n:key='PLUGINS_CORE_USERS_SEARCH_BUTTON' i18n:catalogue='plugin.core'/>",
		...
		// Quit
		this._search.addAction("<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_QUIT' i18n:catalogue='plugin.core'/>", 
					null,
					Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/quit.png', 
	...
	{
		this._actions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE' i18n:catalogue='plugin.core'/>"});

		if (this._modifiable)
		{
			// Add user
			this._actions.addAction("<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_NEW' i18n:catalogue='plugin.core'/>",
					null,
					Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/add_user.png', 
					Ext.bind(this.add, this));
		}
		
		// Edit 
		this._actions.addAction("<i18n:text i18n:key='PLUGINS_CORE_USERS_IMPERSONATE' i18n:catalogue='plugin.core'/>",
					null,
					Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/impersonate_user.png', 
					Ext.bind(this.impersonate, this));

		if (this._modifiable)
		{
			// Edit 
			this._actions.addAction("<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_EDIT' i18n:catalogue='plugin.core'/>",
					null,
					Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/edit_user.png', 
		...
		{
			// Delete 
			this._actions.addAction("<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_DEL' i18n:catalogue='plugin.core'/>",
					null,
					Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/users/delete.png', 
	...
	{
		var helpPanel = new Ametys.admin.rightpanel.TextPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_USERS_HELP' i18n:catalogue='plugin.core'/>"});
		helpPanel.addText("<i18n:text i18n:key='PLUGINS_CORE_USERS_HELP_TEXT_READ' i18n:catalogue='plugin.core'/>");
	...
	impersonate: function ()
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_USERS_IMPERSONATE_ERROR' i18n:catalogue='plugin.core'/>", (result == null || Ext.dom.Query.selectValue("*/error", result, "") != "") ? null : result, "Ametys.plugins.admin.Users.impersonate"))
	    {
	       return;
	    }
	    else
	    {
			...
			Ametys.Msg.show ({
	    		title: "<i18n:text i18n:key='PLUGINS_CORE_USERS_IMPERSONATE_SUCCESS_TITLE' i18n:catalogue='plugin.core'/>",
	    		msg: "<i18n:text i18n:key='PLUGINS_CORE_USERS_IMPERSONATE_SUCCESS' i18n:catalogue='plugin.core'/> " + name + " (" + login + ")<br/><br/><i18n:text i18n:key='PLUGINS_CORE_USERS_IMPERSONATE_SUCCESS_2' i18n:catalogue='plugin.core'/>",
	    		buttons: Ext.Msg.OK,
	...
	add: function ()
	{
		Ametys.plugins.admin.UserEdit.act({"mode": "new"}, Ext.bind(this._addCb, this));
	},
	...
	{
		var elt = this._listView.getSelectionModel().getSelection()[0];
	    Ametys.plugins.admin.UserEdit.act({"mode": "edit", "login" : elt.data.login}, Ext.bind(this._editCb, this));
	}, 
	...
	remove: function ()
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_DEL_TITLE' i18n:catalogue='plugin.core'/>", 
						 "<i18n:text i18n:key='PLUGINS_CORE_USERS_HANDLE_DEL_PROMPT' i18n:catalogue='plugin.core'/>", 
	...
	doRemove: function (answer)
	{
		...
		{
	        var elt = this._listView.getSelectionModel().getSelection()[0];              
	        var url = Ametys.getPluginDirectPrefix(this.pluginName) + "/users/delete";
	        ...
	        if (200 != Ext.Ajax.request({url: url, params: args, async: false}).status)
	        {
	        	Ametys.Msg.show ({
	            		...
	            		msg: "<i18n:text i18n:key='PLUGINS_CORE_USERS_DELETE_ERROR' i18n:catalogue='plugin.core'/>",
	            		buttons: Ext.Msg.OK,
	...
	search: function ()
	{
		...
		var searchValue = searchField.getValue() == '<i18n:text i18n:key="PLUGINS_CORE_USERS_SEARCH_CRITERIA" i18n:catalogue="plugin.core"/>' ? '' : searchField.getValue();
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_USERS_SEARCHING_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Users.search"))
	    {
	       return;
	    }
	    ...
	    {
	    	Ametys.Msg.show ({
	        		title: "<i18n:text i18n:key='PLUGINS_CORE_USERS_SEARCHING' i18n:catalogue='plugin.core'/>",
	        		msg: "<i18n:text i18n:key='PLUGINS_CORE_USERS_SEARCHING_NORESULT' i18n:catalogue='plugin.core'/>",
	        		buttons: Ext.Msg.OK,
						icon: Ext.MessageBox.INFO
	        });
	        return;
	...
	{
		this._listView.getStore().addSorted(Ext.create('Ametys.plugins.admin.Users.Users', {
			'login': login,
