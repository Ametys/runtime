 ...
 * @private
 */
Ext.define('Ametys.plugins.admin.Profiles', {
	singleton: true,
	...
	{
		var newEntry = Ext.create('Ametys.plugins.admin.Profiles.Profile', {
			'name': "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_NEWPROFILE' i18n:catalogue='plugin.core'/>",
			'rights': {},
			'id': 'new'
		}); 
        ...
        var result = Ametys.data.ServerComm.send({
        	...
        	url: "/rights/profiles/create", 
        	parameters: {name: newEntry.get('name') }, 
        	priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
        	callback: null, 
        	resonseType: null
        });
        if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_NEW_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Logs.createProfile"))
        {
            return;
        }
        else
	...
	remove: function()
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_DELETE' i18n:catalogue='plugin.core'/>", "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_DELETE_CONFIRM' i18n:catalogue='plugin.core'/>", Ext.bind(this.removeConfirm, this));
	},
	...
	removeConfirm: function(button)
	{
		if (button == 'yes')
		{
	    	...
	    	if (200 == Ext.Ajax.request({url: Ametys.getPluginDirectPrefix(this.pluginName) + "/rights/profiles/delete", params: "id=" + elt.get('id'), async: false}).status)
			{
			...
			else
			{
				Ametys.Msg.show ({
            		...
            		msg: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_ERROR' i18n:catalogue='plugin.core'/>",
            		buttons: Ext.Msg.OK,
    ...
    saveObjects: function(element, objects, newRights)
    {
		...
		while (!ok)
		{
			var result = Ametys.data.ServerComm.send({
				plugin: this.pluginName, 
				url: "/rights/profiles/modify", 
				parameters: { id: element.get('id'), objects: objects }, 
				priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
				callback: null, 
				responseType: null
			});
		    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_MODIFY_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Logs.saveObjects"))
		    {
				return;
		    }
		    else
			{
				...
				if (state != null && state == "missing")
				{
					Ametys.Msg.show ({
                		title: "<i18n:text i18n:key='PLUGINS_ADMIN_ERROR_DIALOG_TITLE'/>",
                		msg: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_MODIFY_MISSING_ERROR' i18n:catalogue='plugin.core'/>",
                		buttons: Ext.Msg.OK,
	   					icon: Ext.MessageBox.ERROR
                	});
    ...
    checkBeforeQuit: function()
	{
		...
		{
			if (confirm("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_MODIFY_CONFIRM' i18n:catalogue='plugin.core'/>"))
			{
	...
	validateEdit: function(e)
	{
		...
		if (!/^[a-z|A-Z|0-9| |-|_]*$/.test(e.value))
        {
        	Ametys.Msg.show ({
                		title: "<i18n:text i18n:key='PLUGINS_ADMIN_ERROR_DIALOG_TITLE'/>",
                		msg: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_NAMING_ERROR' i18n:catalogue='plugin.core'/>",
                		buttons: Ext.Msg.OK,
	   					icon: Ext.MessageBox.ERROR
                	});
            return false;
	...
	editLabel: function(editor, e, eOpts)
	{
		if (e.record.get('id') == "new")
		{
		    ...
		    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_NEW_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Logs.editLabel"))
		    {
		       return;
		    }
			else
		...
		else
		{
			...
			var result = Ametys.data.ServerComm.send({
				plugin: this.pluginName, 
				url: "/rights/profiles/rename", 
				parameters: { id: e.record.get('id'), name: e.record.get('name') }, 
				priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
				callback: null, 
				responseType: null
			});
		    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_RENAME_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Logs.editLabel"))
		    {
		       return;
		    }
			else 
			{
				...
				if (state != null && state == "missing")
				{
					Ametys.log.ErrorDialog.display({
						title: "<i18n:text i18n:key='PLUGINS_ADMIN_ERROR_DIALOG_TITLE'/>", 
						text: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_RENAME_MISSING_ERROR' i18n:catalogue='plugin.core'/>", 
	...
	selectLayout: function()
	{
		...
		{
			Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_ADMIN_SAVE_DIALOG_TITLE' i18n:catalogue='plugin.core'/>", "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_MODIFY_CONFIRM' i18n:catalogue='plugin.core'/>", Ext.bind(this.saveObjectConfirm, this, [this._selectedElmt, this._getObjects(), this._getRights()], true));
		}
	...
	{
		// Tabs
		this._Navigation = new Ametys.admin.rightpanel.NavigationPanel ({title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_MENU'/>"});
		this.item1 = new Ametys.admin.rightpanel.NavigationPanel.NavigationItem ({
			text: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILE_READ' i18n:catalogue='plugin.core'/>",
		...
		this.item2 = new Ametys.admin.rightpanel.NavigationPanel.NavigationItem ({
			text: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILE_EDIT' i18n:catalogue='plugin.core'/>",
			handlerFn: Ext.bind(this.selectLayout, this),
			activeItem: 1,
			cardLayout: 'profile-card-panel',
			toggleGroup : 'profile-menu'
		...
		// Handling profiles
		this._Category = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_CATEGORY' i18n:catalogue='plugin.core'/>"});
		this._Category.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_CREATE' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/new.png", Ext.bind(this.createProfile, this));
		this._Category.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_RENAME' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/rename.png", Ext.bind(this.rename, this));
		this._Category.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_VALIDATE' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/validate.png", Ext.bind(this.saveObjects, this, []));
		this._Category.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_DELETE' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/delete.png", Ext.bind(this.remove, this));
		this._Category.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_QUIT' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/quit.png", Ext.bind(this.goBack, this));
		...
		// Utils
		this._utilsCategory = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_SELECT_HANDLE_CATEGORY' i18n:catalogue='plugin.core'/>"});
		this._utilsCategory.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_SELECT_ALL' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/select_all.png", Ext.bind(this.selectAll, this, []));
		this._utilsCategory.addAction("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_UNSELECT_ALL' i18n:catalogue='plugin.core'/>", null, Ametys.getPluginResourcesPrefix('admin-old') + "/img/profiles/unselect_all.png", Ext.bind(this.unselectAll, this, []));
		this._utilsCategory.setVisible(false);
		
		// Help
		var helpCategory = new Ametys.admin.rightpanel.TextPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HELP_CATEGORY' i18n:catalogue='plugin.core'/>"});
		helpCategory.addText("<i18n:text i18n:key='PLUGINS_CORE_RIGHTS_PROFILES_HANDLE_HELP_HINT' i18n:catalogue='plugin.core'/>");
		
		Ext.define('Ametys.plugins.admin.Profiles.Profile', {
 ...
 */
Ext.define('Ametys.plugins.admin.Profiles.RightEntry', {
	...
	onRender: function(ct, position)
	{
		...
		if (this.description)
		{
			var img = this.getEl().createChild({
				...
				cls: 'right-entry-img',
				tag:'img',
				src: Ametys.getPluginResourcesPrefix('admin-old') + '/img/config/help.gif'});
 ...
 * @private
 */
Ext.define('Ametys.plugins.admin.Profiles.CheckRightEntry', {
