 ...
 * @private
 */
Ext.define('Ametys.plugins.admin.Config', {
	singleton: true,
	...
	* @private
	...
	_categories: {},
	/**
	 * @property {Ametys.plugins.admin.Config.ParameterChecker[]} _paramCheckers all the parameter checkers of this page
	 */
	_paramCheckers: [],
	/**
	...
	_widgets: {
		...
		'edition.textarea': 'Ametys.plugins.admin.Config.TextareaField'
	},
	...
	createPanel: function ()
	{
		...
		this._form  = new Ext.form.FormPanel({
			...
			fieldDefaults: {
				cls: 'ametys',
				labelAlign: 'right',
		        labelWidth: Ametys.plugins.admin.Config.LABEL_WIDTH,
		        labelSeparator: ''
			},
	...
	createFieldSet: function (id, label)
	{
		...
		return new Ext.panel.Panel({
				...
				width: Ametys.plugins.admin.Config.LABEL_WIDTH
					   + Ametys.plugins.admin.Config.FIELD_WIDTH
					   + Ametys.plugins.admin.Config.AMETYS_DESCRIPTION_WIDTH
		});
	},
	...
	addGroupCategory: function (fd, id, name, switcher, subitems)
	{
		if (switcher != null)
		{
			...
			var items = [
			      input,
			      { width: Ametys.plugins.admin.Config.SWITCHER_WIDTH, baseCls: '', html: "<label for='" + input.getInputId() + "'>" + name + "</label>", padding: "3 0 0 5" }
            ];
			...
			if (switcher.description != '')
			{
				items.push({
				    ...
				    padding: "3 0 0 5",
					html: '  ',
					width: Ametys.plugins.admin.Config.AMETYS_DESCRIPTION_WIDTH,
					listeners: {
						'render': function() {
						    new Ext.ToolTip({
						        target: this.getEl(),
	...
	{
		var parameterChecker = new Ametys.plugins.admin.Config.ParameterChecker(paramChecker, categoryLabel, "category", label, description),
		...
		testContainer.setMargin(Ametys.plugins.admin.Config.TEST_CONTAINERS_TOP_MARGIN + ' 0 ' +  Ametys.plugins.admin.Config.TEST_CONTAINERS_BOTTOM_MARGIN + ' ' + Ametys.plugins.admin.Config.CHECKERS_LEFT_MARGIN);
	...
	{
		var parameterChecker = new Ametys.plugins.admin.Config.ParameterChecker(paramChecker, categoryLabel+"/"+groupLabel, "group", label, description),
		    testButton = this._generateTestButton(parameterChecker),
		    testContainer = this._generateTestContainer(parameterChecker, testButton);
	...
	addParameterChecker: function(paramChecker, categoryLabel, groupLabel, label, description)
	{
	    ...
	    var parameterChecker = new Ametys.plugins.admin.Config.ParameterChecker(paramChecker, categoryLabel+"/"+groupLabel+"/"+uiRefLabel, "parameter", label, description),
	    	testButton = this._generateTestButton(parameterChecker),
	    	testContainer = this._generateTestContainer(parameterChecker, testButton);
	...
	{	
		...
		return new Ext.button.Button({
			...
			text: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_LABEL_PREFIX'/>" + " \"" + paramChecker.label + "\"",
...
//					  	  '</td>' +
//			  	  		  '<td>' +
//			  	  		  	'<em style="font-size: x-small; text-align: center">' + "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_LABEL_PREFIX'/>" + " \"" + paramChecker.label + "\"" + '</em>' +
//					  	  '</td>' +
//					  '</tr>' +
//				  '</table>',
		...
		tpl = new Ext.XTemplate(
								'<table>' + 
									  ...
									  '<tr>' +
									   ...
									   '<tr>' +
									     	'<td colspan="2">' +
								   				"- <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_TOOLTIP_TESTED_PARAMS'/>" + paramChecker.linkedParamsLabels +
								   			'</td>' +
									   '</tr>' +
								'</table>'
								);
	...
	_generateStatusTip: function(el, paramChecker)
    {
		var tipHtml = [],
			tpl = new Ext.XTemplate(
						   					"<tpl if='status == " + Ametys.plugins.admin.Config.ParameterChecker.STATUS_NOT_TESTED + "'><i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_STATUS_NOT_TESTED'/></tpl>" +
								   			"<tpl if='status == " + Ametys.plugins.admin.Config.ParameterChecker.STATUS_SUCCESS + "'><i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_STATUS_SUCCESS'/></tpl>" +
								   			"<tpl if='status == " + Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE + "'>" +
								   				"<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_STATUS_FAILURE'/> </br>" +
								   				'<tr>' + 
								   					'<td colspan="2">' +
								   						"- <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_TOOLTIP_MESSAGE'/> <strong>" + paramChecker.errorMsg + "</strong>" +
							   						'</td>' +
						   						'</tr>' +
							   				"</tpl>" +
							   				"<tpl if='status == " + Ametys.plugins.admin.Config.ParameterChecker.STATUS_DEACTIVATED + "'>" +
							   					"<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_STATUS_DEACTIVATED'/> </br>" +
							   					'<tr>' + 
							   						'<td colspan="2">' +
							   							"<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_TOOLTIP_MESSAGE'/>" +
							   							"<strong><i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_TOOLTIP_DEACTIVATED_MESSAGE'/></strong>" +
														'</td>' +
						   						'</tr>' +
											"</tpl>" +
						   					"<tpl if='status == " + Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING + "'>" +
								   				"<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_STATUS_WARNING'/> </br>" +
								   				"<tpl if='errorMsg != null'>" +
							  		   				'<tr>' + 
							  		   					'<td colspan="2">' +
							  		   						"<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECKER_TOOLTIP_MESSAGE2'/> <strong>" + paramChecker.errorMsg + "</strong>" +
								   						'</td>' +
							   						'</tr>' +
						   						'</tpl>' +
						   					'</tpl>'
		...
		if (paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING)
		{
		    Ext.form.Labelable.initWarnTip();
			el.set({"data-warnqtip": tip}); 
		}
		else if (paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE)
		{
			el.set({"data-errorqtip": tip});
		}
		else if (paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_DEACTIVATED)
		{
		    Ext.form.Labelable.initWarnTip();
			el.set({"data-warnqtip": tip}); 
		}
	...
	_updateTestResults: function()
	{
		...
		for (var i = 0; i < nbTests; i++)
		{
			...
			switch (status)
			{
				case Ametys.plugins.admin.Config.ParameterChecker.STATUS_SUCCESS: 
					successes += 1;
					break;
					
				case Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE: 
					failures += 1;
					break;
				
				case Ametys.plugins.admin.Config.ParameterChecker.STATUS_NOT_TESTED:
					notTested += 1;
					break;
				
				case Ametys.plugins.admin.Config.ParameterChecker.STATUS_DEACTIVATED:
					notTested += 1;
					break;
					
				case Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING:
					notTested += 1;
					break;
	...
	_updateTestButton: function(paramChecker)
	{
    	...
    	{
	    	btn.enable();
	    	paramChecker.setStatus(Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING);
    	}
	    else
    	{
	    	btn.disable();
			paramChecker.setStatus(Ametys.plugins.admin.Config.ParameterChecker.STATUS_DEACTIVATED);
    	}
	...
	_updateWarnings: function()
	{
		for (var i = 0; i < this._paramCheckers.length; i++)
		{
			var paramChecker = this._paramCheckers[i],
				...
				helpBox = Ext.getCmp(paramChecker.helpBoxId),
				success = paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_SUCCESS ? true : false,
				failure = paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE ? true : false,
				warning = paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING ? true : false,
				warningMsg = "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_WARNING_TEXT_BEGINNING'/>" + paramChecker.label +
						  "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_WARNING_TEXT_END'/>";	
			
			for (var j = 0; j < paramChecker.linkedParams.length; j++)
			{
				var linkedParam = paramChecker.linkedParams[j],
				linkedParamField = Ametys.plugins.admin.Config._form.getForm().findField(linkedParam);
	 ...
	 * Calls the server-side with the configuration parameters.
 	 * @param {Ametys.plugins.admin.Config.ParameterChecker[]} paramCheckers the parameter checkers
	...
	_check: function(paramCheckers, displayErrors, callback, forceTest)
	{
		...
		var params = this._getAllFormValues(),
			url = Ametys.getPluginDirectPrefix(this.pluginName) + "/config/test",
		...
		{	
			...
			{
				btn.disable();
				if (forceTest || paramChecker.getStatus() != Ametys.plugins.admin.Config.ParameterChecker.STATUS_SUCCESS)
				{
					btn.getEl().mask("");
	...
	{	
		if (!success)
		{
			Ametys.log.ErrorDialog.display({
				title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_SERVER_CALL_ERROR_TITLE'/>", 
				text: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_SERVER_CALL_ERROR_TEXT'/>",
	    		category: "Ametys.plugins.admin.Config._checkCb"
			});
		...
		for (var i = 0; i < paramCheckers.length; i++)
		{
		    ...
		    if (errorMsg == null)
			{
		    	...
		    	btn.addCls('success');
		    	helpBox.addCls('offset');
		    	paramChecker.setStatus(Ametys.plugins.admin.Config.ParameterChecker.STATUS_SUCCESS);
		    ...
		    else
			{
		    	...
		    	btn.addCls('failure');
		    	helpBox.addCls('offset');
		    	paramChecker.setStatus(Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE);
	...
	_createDoubleField: function (name, value, label, description, mandatory, regexp, invalidText, width, disableCondition)
	{
		...
		return	new Ext.form.field.Number({
	        ...
	        {	
	    		...
	    		else
	    		{
	    	        ...
	    	        if (trimmed.length < 1 || (value === this.emptyText && this.valueContainsPlaceholder)) 
	    	        {
	    	        	if (mandatory) 
	    	        	{
	    	        		return this.invalidText || "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_INVALID_FIELD_TEXT'/>"; 
	    	            }
	    	        }
	    	        return true;
	    		}
	        ...
	        width: width || Ametys.plugins.admin.Config.FIELD_WIDTH + Ametys.plugins.admin.Config.LABEL_WIDTH + Ametys.plugins.admin.Config.AMETYS_DESCRIPTION_WIDTH
		});
	},
...
//	    }
		...
		return new Ext.form.field.Date ({
	        ...
	        disableCondition: disableCondition != null ? disableCondition : null,
	        regexText: invalidText != null ? invalidText :  "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_INVALID_REGEXP'/>"  + regexp,
			invalidText: invalidText != null ? invalidText : null,
			msgTarget: 'side',
	...
	{
		this._navigationPanel = new Ametys.admin.rightpanel.NavigationPanel ({title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_MENU'/>"});
		...
		for (var categoryLabel in this._navigationMap)
		{
			...
			if (!invalidField)
			{
				...
				for (var i = 0; i < paramCheckers.length; i++)
				{
					...
					{
						if (paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE) 
						{
							navItem.removeCls('navigation-item-warning');
							navItem.addCls('navigation-item-error');
							error = true;
						...
						else if (warningField || paramChecker.getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING)
						{
							navItem.removeCls('navigation-item-error');
							navItem.addCls('navigation-item-warning');
							warning = true;
	 ...
	 * @returns {Ametys.plugins.admin.Config.ParameterChecker[]} the parameter checkers belonging to this category
	 */
	...
	{
		this._actions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_HANDLE'/>"});
		
		// Save action
		this._actions.addAction("<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_HANDLE_SAVE'/>",
						 null,
						 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/config/save.png',
						 Ext.Function.bind(this.save, this));
		
		// Quit action
		this._actions.addAction("<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_HANDLE_QUIT'/>",
						 null,
						 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/config/quit.png',
	...
	{
		this._testResults = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_TEST_RESULTS_TITLE'/>"});
		
		// Initialize the test results text
		this._testResults.addText('');
		
		// Run all tests action
		this._testResults.addAction("<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_RUN_ALL_TESTS'/>",
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/config/testall.png',
				 Ext.bind(this._check, this,  [this._paramCheckers, true, null, true]));
		// Relaunched missed tests action
		this._testResults.addAction("<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_RUN_MISSED_TESTS'/>",
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/config/testmissing.png',
	...
	_updateTestResultsPanel: function(testResults)
	{
        ...
        tpl = new Ext.Template(
        			"<div class='test-results'>" + 
	        			"<span><i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_TEST_RESULTS_TEXT'/></span>" +
						'<ul>' + 
							"<li> {successes} <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_TEST_RESULTS_SUCCESSES'/></li>" +
							"<li> {failures} <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_TEST_RESULTS_FAILURES'/></li>" +
							"<li> {notTested} <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_TEST_RESULTS_NOT_TESTED'/></li>" +
						'</ul>' +
					"</div>"
	    			);
		...
		for (var i = 0; i < this._paramCheckers.length; i++)
		{
			...
			if(this._paramCheckers[i].getStatus() == Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING)
			{
				warning = true;
			}
		}
	...
	{
		var helpPanel = new Ametys.admin.rightpanel.TextPanel({title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_HELP'/>"});
		helpPanel.addText("<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_HELP_TEXT'/>");
	...
	save: function ()
	{
		...
		for (var i = 0; i < this._paramCheckers.length; i++)
		{
			...
			if (Ext.getCmp(paramChecker.buttonId).isVisible() && ( status == Ametys.plugins.admin.Config.ParameterChecker.STATUS_FAILURE 
					  											|| status == Ametys.plugins.admin.Config.ParameterChecker.STATUS_NOT_TESTED
		  														|| status == Ametys.plugins.admin.Config.ParameterChecker.STATUS_WARNING))
			{
				testsOk = false;
			}
		}
		...
		if (!testsOk)
		{
			...
			msgBox.show({
						...
						icon: Ext.Msg.ERROR,
						height: Ametys.plugins.admin.Config.LAUNCH_TESTS_SAVE_MBOX_HEIGHT,
						fn: function(btn)
						{
							if (btn == 'yes')
							{
	...
	_save2: function ()
	{
	    var url = Ametys.getPluginDirectPrefix(this.pluginName) + "/config/set",
	...
	_displayErrorDialog: function()
	{
		...
		for (var i = 0; i < this._paramCheckers.length; i++)
		{
			...
			if (Ext.getCmp(paramChecker.buttonId).isVisible())
			{
				...
				{
					case 'category':
						firstSentence = "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECK_UI_REF_CATEGORY'/>";
						break;
				
					case 'group':
						firstSentence = "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECK_UI_REF_GROUP'/>";
						break;
						
					case 'parameter':
						firstSentence = "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECK_UI_REF_PARAMETER'/>";
						break;
		...
		{
			Ametys.log.ErrorDialog.display({
				title: "<i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECK_ERROR'/>", 
				text: nbErrors + " <i18n:text i18n:key='PLUGINS_ADMIN_CONFIG_PARAM_CHECK_ERROR_TEXT'/>",
	    		category: "Ametys.plugins.admin.Config._runAllTests",
	    		details: details
			});
		}
	},
 ...
 */
Ext.define('Ametys.plugins.admin.Config.TimeField',	{
 ...
 */
Ext.define('Ametys.plugins.admin.Config.HourField',	{
	extend: 'Ametys.plugins.admin.Config.TimeField',
 ...
 */
Ext.define('Ametys.plugins.admin.Config.TextareaField',	{
	extend: 'Ext.form.field.TextArea',
	
	width: Ametys.plugins.admin.Config.FIELD_WIDTH + Ametys.plugins.admin.Config.LABEL_WIDTH,
    height: 80
});
 ...
 */
Ext.define('Ametys.plugins.admin.Config.ParameterChecker', {
