...
Ext.define('Ametys.plugins.admin.Groups', {
	singleton: true,
	...
	{
		Ext.define('Ametys.plugins.admin.Groups.Group', {
		...
		Ext.define('Ametys.plugins.admin.Groups.Users', {
		    extend: 'Ext.data.Model',
		    fields: [
		...
		this._listViewGp = new Ext.grid.Panel({
			...
			title : "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_LABEL' i18n:catalogue='plugin.core'/>",
			hideHeaders: true,
			border: false,
			autoScroll: true,
			
		    store: Ext.create('Ext.data.Store', {
				model: 'Ametys.plugins.admin.Groups.Group',
		        data: { groups: []},
		...
		this._listViewU = new Ext.grid.Panel({
			...
			title : "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_COLUMN' i18n:catalogue='plugin.core'/>",
			hideHeaders: true,
			multiSelect: true,
			
			store : Ext.create('Ext.data.Store', {
				model: 'Ametys.plugins.admin.Groups.Users',
		        data: { users: []},
	...
	_selectGroup: function (me, record, index, eOpts)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_LIST_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Groups._selectGroup"))
	    {
		...
		for (var i=0; i < members.length; i++)
		{
			...
			var newEntry = Ext.create('Ametys.plugins.admin.Groups.Users', {
				'user': fullname + " (" + login + ")",
				'id': login
			}); 
	...
	_editGroupLabel: function (editor, e, eOpts)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_RENAME_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Groups._editGroupLabel"))
	    {
	       // nothing
	    }
		else
		{
			...
			if (state != null && state == "missing")
			{
				Ametys.Msg.show ({
            		...
            		msg: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_RENAME_MISSING_ERROR' i18n:catalogue='plugin.core'/>",
            		buttons: Ext.Msg.OK,
	...
	addUser: function () 
	{
		function cb (users)
		{
			...
			for (var i in users)
			{
				...
				if (e == -1)
				{
					e = Ext.create('Ametys.plugins.admin.Groups.Users', {
						'user': users[i],
						'id': i
					}); 
	...
	save: function (group, objects)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_MODIFY_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Groups.save"))
	    {
		...
		else
		{
			...
			if (state != null && state == "missing")
			{
				Ametys.Msg.show ({
	            		title: "<i18n:text i18n:key='PLUGINS_ADMIN_ERROR_DIALOG_TITLE'/>",
	            		msg: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_MODIFY_MISSING_ERROR' i18n:catalogue='plugin.core'/>",
	            		buttons: Ext.Msg.OK,
	   					icon: Ext.MessageBox.ERROR
	            });
	...
	{
		this._groupsActions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_CATEGORY' i18n:catalogue='plugin.core'/>"});

		if (this._modifiable)
		{
			// New group
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_CREATE' i18n:catalogue='plugin.core'/>",
					 null,
					 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/add_group.png', 
					 Ext.bind(this.add, this));
			
			// Rename goup
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_RENAME' i18n:catalogue='plugin.core'/>",
					 null,
					 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/rename.png', 
					 Ext.bind(this.rename, this));
			
			// Delete goup
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_DELETE' i18n:catalogue='plugin.core'/>",
					 null,
					 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/delete.png', 
					 Ext.bind(this.remove, this));
		}
		
		// Quit
		this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_QUIT' i18n:catalogue='plugin.core'/>",
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/quit.png', 
	...
	_drawUsersActionsPanel: function ()
	{
		...
		this._usersActions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_USERS_HANDLE_CATEGORY' i18n:catalogue='plugin.core'/>"});

		// Add user
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_ADDUSER' i18n:catalogue='plugin.core'/>",
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/add_user.png', 
				 Ext.bind(this.addUser, this));
		
		// Delete user
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_DELETEUSER' i18n:catalogue='plugin.core'/>", 
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/delete.png', 
				 Ext.bind(this.deleteUsers, this));
		
		// Validate modification
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_VALIDATE' i18n:catalogue='plugin.core'/>", 
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/validate.png', 
	...
	{
		var helpPanel = new Ametys.admin.rightpanel.TextPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HELP_CATEGORY' i18n:catalogue='plugin.core'/>"});
		helpPanel.addText("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_HELP_HINT' i18n:catalogue='plugin.core'/>");
	...
	add: function ()
	{
		...
		var result = Ametys.data.ServerComm.send({
			...
			url: "/groups/create", 
			parameters: { name: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_NEWGROUP' i18n:catalogue='plugin.core'/>" }, 
			priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
			callback: null, 
			responseType: null
		});
		
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_NEW_ERROR' i18n:catalogue='plugin.core'/>", result, "Ametys.plugins.admin.Groups.add"))
	    {
	       return false;
	    }
		...
		var newEntry = Ext.create('Ametys.plugins.admin.Groups.Group', {
			'name': "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_NEWGROUP' i18n:catalogue='plugin.core'/>",
			'id': id
		}); 
	...
	remove: function () 
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_HANDLE_DELETE' i18n:catalogue='plugin.core'/>", 
	...
	doRemove: function (answer)
	{
		if (answer == 'yes')
		{
	    	...
	    	if (200 == Ext.Ajax.request({url: Ametys.getPluginDirectPrefix('core') + "/groups/delete", params: "id=" + elt.get('id'), async: false}).status)
			{
			...
			else
			{
				Ametys.Msg.show ({
	        		title: "<i18n:text i18n:key='PLUGINS_ADMIN_ERROR_DIALOG_TITLE'/>",
	        		msg: "<i18n:text i18n:key='PLUGINS_CORE_GROUPS_DELETE_ERROR' i18n:catalogue='plugin.core'/>",
	        		buttons: Ext.Msg.OK,
					icon: Ext.MessageBox.ERROR
	        	});
			}
	...
	checkBeforeQuit: function()
    {
        if (this._currentGroup != null && this._hasChanges)
        {
            if (confirm("<i18n:text i18n:key='PLUGINS_CORE_GROUPS_MODIFY_CONFIRM' i18n:catalogue='plugin.core'/>"))
            {
                this.save();
            }
        }
