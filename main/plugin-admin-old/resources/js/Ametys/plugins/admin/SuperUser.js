 ...
 * @private
 */
Ext.define('Ametys.plugins.admin.SuperUser', {
	singleton: true,
	...
	_affectSuperUser: function (users) 
	{
		...
		var response = Ametys.data.ServerComm.send({
			...
			url: "/superuser/affect", 
			parameters: { login: logins }, 
			priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
			callback: null, 
			responseType: null
		});
		if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_ADMIN_SUPERUSER_AFFECT_ERROR'/>", response, "Ametys.plugins.admin.SuperUser._affectSuper"))
	    {
	       return;
	    }
		
		Ametys.Msg.show ({
			title: "<i18n:text i18n:key='PLUGINS_ADMIN_SUPERUSER_AFFECT_TITLE'/>",
			msg: "<i18n:text i18n:key='PLUGINS_ADMIN_SUPERUSER_AFFECT_SUCCESS'/>",
			buttons: Ext.Msg.OK,
