 ...
 * @private
 */
Ext.define('Ametys.plugins.core.userprefs.UserPrefsActions', {
	singleton: true,
	...
	{
		Ametys.Msg.show({
		    title: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_RESET_WORKSPACE_PROMPT_TITLE'/>",
		    msg: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_RESET_WORKSPACE_PROMPT_DESC'/>",
	...
	_workspaceResetedCB: function(success, errors)
	{
    	if (success == false && errors != null)
    	{
    	    ...
    	    Ametys.log.ErrorDialog.display({
    	        title: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_RESET_WORKSPACE_FAILURE_TITLE'/>",
    	        text: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_RESET_WORKSPACE_FAILURE_DESC'/>",
    	        details: details,
	...
	_delayedInitializeUserPrefsDialog: function ()
	{
		...
		if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_ERROR'/>", response, Ext.getClassName(this)))
        {
            return false;
        }
		...
		this._box = Ext.create('Ametys.window.DialogBox', {
			title :"<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_LABEL'/>",
			...
			closeAction: 'hide',
			buttons : [ {
				text :"<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_BTN_RESTORE_DEFAULTS'/>",
				handler : this.restoreDefaults,
				scope: this
			}, ' ', ' ',{
				text :"<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_BTN_OK'/>",
				handler : this._saveUserPrefs,
				scope: this
			}, {
				text :"<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_BTN_CANCEL'/>",
	...
	_saveUserPrefs: function ()
	{
		...
		{
			Ametys.Msg.show({
				   title: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_SAVE'/>",
				   msg: "<i18n:text i18n:key='PLUGINS_CORE_USER_PREFERENCES_SAVE_INVALIDFIELDS'/>" + invalidFields.join(', '),
				   buttons: Ext.Msg.OK,
