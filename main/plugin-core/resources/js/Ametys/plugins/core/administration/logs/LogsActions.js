 ...
 * @private
 */
Ext.define('Ametys.plugins.core.administration.logs.LogsActions', {
	singleton: true,
	...
	view: function(controller)
	{
		...
		if (target != null)
		{
		    ...
		    {
				window.location.href = Ametys.getPluginDirectPrefix('core') + "/administrator/logs/view/" + encodeURIComponent(targetParameters.location);
		    }
		}
	},
	...
	{
	    var url = Ametys.getPluginDirectPrefix('core') + "/administrator/logs/download.zip";
	    var args = "";
	...
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_DELETE_DIALOG_TITLE'/>", 
							"<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_DELETE_DIALOG_CONFIRM'/>", 
	...
	_delete: function(answer, controller)
	{
		if (answer == 'yes')
	    {
	        ...
	        controller.serverCall('deleteLogs', [filesLocations], Ext.bind(this._deleteCb, this),
				{ errorMessage: { msg: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_DELETE_ERROR'/>", category: 'Ametys.plugins.core.administration.actions.LogsActions'} }, 
			true);
	    }
	},
	...
	_deleteCb: function(response)
	{
        ...
        {
        	Ametys.Msg.show({
				title: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_DELETE_DIALOG_TITLE'/>",
				msg: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_DELETE_ERROR'/>",
				buttons: Ext.Msg.OK,
	...
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE_DIALOG_TITLE'/>", 
					         "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE_DIALOG_CONFIRM'/>",
	...
	_purge: function (answer, controller)
	{
		if (answer == 'yes')
	    {
	        controller.serverCall('purgeLogs', null, Ext.bind(this._purgeCb, this),
				{ errorMessage: { msg: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE_SERVER_ERROR'/>", category: 'Ametys.plugins.core.administration.actions.LogsActions'} }, 
			true);
	    }	
	},
	...
	_purgeCb: function(response)
	{
        ...
        Ametys.Msg.show({
			title: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE'/>",
			msg: deletedFiles.length == 0 ? "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE_NONE'/>" : deletedFiles.length + " " + "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_LOGS_PURGE_AMOUNT'/>",
			buttons: Ext.Msg.OK,
	...
	/**
	 * Change the logging level of the selected category to the configured #Ametys.plugins.core.administration.controller.LogLevelController._level
	 * @param {Ametys.ribbon.element.ui.ButtonController} controller The controller calling this function
	 */
	changeLogLevel: function(controller)
	{
		...
		if (targets.length > 0)
		{
			...
			controller.serverCall('changeLogLevel',
					...
					{ 
						errorMessage: { 
							msg: "<i18n:text i18n:key='PLUGINS_CORE_ADMINISTRATOR_TOOL_LOGS_LEVEL_SERVER_ERROR'/>", 
							category: 'Ametys.plugins.core.administration.actions.LogsActions' 
						},
						arguments: {
							level: newLevel,
							category: category
