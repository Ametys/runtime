...
<xsl:stylesheet version="1.0" 
                ...
                xmlns:captcha="org.ametys.runtime.captcha.CaptchaHelper"
	...
	<xsl:template name="captcha">
        ...
        <xsl:variable name ="captcha-type"><xsl:value-of select="captcha:getCaptchaType()" /></xsl:variable>
	
        <xsl:choose>
            <xsl:when test="$captcha-type = 'jcaptcha'">
            ...
            <xsl:when test="$captcha-type = 'recaptcha'">
    ...
    <xsl:template name="captcha_impl">
        ...
        <script type="text/javascript">
            ...
            function <xsl:value-of select="$js-funcname-torefresh"/>(focusNow)
            {
                ...
                document.getElementById("<xsl:value-of select="$image-id"/>").src = "<xsl:value-of select="$contextPath"/>/plugins/<xsl:value-of select="$plugin"/>/captcha/" + newId + "/image.png?cancelledKey=" + oldId + "&amp;width=<xsl:value-of select="$image-width"/>" + "&amp;height=<xsl:value-of select="$image-height"/>";
    ...
    <xsl:template name="recaptcha_impl">
        ...
        <xsl:variable name="public-key"><xsl:value-of select="captcha:getReCaptchaPublicKey()" /></xsl:variable>
