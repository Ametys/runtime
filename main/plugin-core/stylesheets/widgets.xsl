...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1">
    ...
    <xsl:template name="select-i18n-input">
        <xsl:param name="class"/>
        <xsl:param name="style"/>
        <xsl:param name="values"/>
        <xsl:param name="value"/>
        ...
        <select name="{$name}" id="{$name}" class="{$class}" style="{$style}">
            ...
            <xsl:for-each select="$values/option">
                <option value="{value}"><xsl:if test="$value=value"><xsl:attribute name="selected">selected</xsl:attribute></xsl:if>
                    <xsl:choose>
                        <xsl:when test="label/@i18n = 'true'">
                            <xsl:text></xsl:text><i18n:text i18n:key="{label/key}" i18n:catalogue="{label/catalogue}"/><xsl:text></xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
