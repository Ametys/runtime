...
public class StaticUIItemFactory extends AbstractLogEnabled implements UIItemFactory, Configurable, ThreadSafe, Serviceable, PluginAware
{
    ...
    /**
     * Returns all declared UIItem, without further checking
     * @return all declared UIItem, without further checking
     */
    public List<UIItem> getUIItemsDefinitions()
    {
        return Collections.unmodifiableList(_itemsList);
    }
    ...
    /**
     * Determine following the right parameter if the user has right to have
     * this static entry
     * 
     * @param right The right name to check. Can be null.
     * @return true if the user has the right or if there is not right and false
     *         otherwise
     */
     ...
     * @throws ConfigurationException If the message is not configuration
     *             properly
     */
     ...
     * @throws ConfigurationException If the shortcut configuration is present
     *             but incorrect
     */
    ...
    protected IconSet _configureIconSet(String rootElementName, Configuration configuration) throws ConfigurationException
    {
        ...
        String pluginName = iconset.getAttribute("plugin", _pluginName);
        ...
        return new IconSet("/plugins/" + pluginName + "/resources", smallIcon, mediumIcon, largeIcon);
    }
    ...
    protected Action _configureAction(Configuration configuration) throws ConfigurationException
    {
        ...
        {
            String pluginName = actionConfiguration.getAttribute("plugin", _pluginName);
            ...
            if ("url".equals(type))
            {
                ...
                {
                    String mode = actionConfiguration.getChild("Url").getAttribute("mode", "plugin-wrapped");
                    return Action.createLinkAction(pluginName, url, mode);
                }
            }
            else if ("function".equals(type))
            {
                ...
                {
                    Set<String> imports = _configureActionImports(pluginName, actionConfiguration.getChild("Imports", false));
                    Map<String, String> parameters = _configureActionParameters(actionConfiguration.getChild("Function", false));

                    return Action.createFunctionAction(pluginName, url, imports, parameters);
                }
            }
            else if ("class".equals(type))
            {
                ...
                return Action.createClassAction(pluginName, url, imports, parameters);
            }
            else
            {
    ...
    private Set<String> _configureActionImports(String pluginName, Configuration configuration) throws ConfigurationException
    {
        if (configuration != null)
        {
            ...
            for (int i = 0; i < filesConfiguration.length; i++)
            {
                ...
                {
                    String localPluginName = filesConfiguration[i].getAttribute("plugin", pluginName);
                    
                    imports.add("/plugins/" + localPluginName + "/resources/" + value);
                }
            }
