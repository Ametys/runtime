...
public class JdbcUsersManager extends AbstractLogEnabled implements UsersManager, Configurable, ThreadSafe, Component, Serviceable, Contextualizable, PluginAware
{
    ...
    protected JdbcParameter _configureParameter(String id, String column, Configuration configuration) throws ConfigurationException
    {
        ...
        try
        {
            if ("login".equals(id))
            {
                ByteArrayInputStream validatorStream = new ByteArrayInputStream("<Validation><mandatory/><regexp>^[\\w]{3,16}$</regexp></Validation>".getBytes());
                ...
                parameter = new JdbcParameter(BASE_PLUGIN_NAME, id, column, "PLUGINS_CORE_USERS_JDBC_FIELD_LOGIN_LABEL", "PLUGINS_CORE_USERS_JDBC_FIELD_LOGIN_DESCRIPTION",
                        ParameterHelper.TYPE.STRING, null, null, validator);
            }
            else if ("lastname".equals(id))
            {
                ...
                parameter = new JdbcParameter(BASE_PLUGIN_NAME, id, column, "PLUGINS_CORE_USERS_JDBC_FIELD_LASTNAME_LABEL", "PLUGINS_CORE_USERS_JDBC_FIELD_LASTNAME_DESCRIPTION",
                        ParameterHelper.TYPE.STRING, null, null, validator);
            }
            else if ("firstname".equals(id))
            {
                ...
                parameter = new JdbcParameter(BASE_PLUGIN_NAME, id, column, "PLUGINS_CORE_USERS_JDBC_FIELD_FIRSTNAME_LABEL", "PLUGINS_CORE_USERS_JDBC_FIELD_FIRSTNAME_DESCRIPTION",
                        ParameterHelper.TYPE.STRING, null, null, validator);
            }
            else if ("email".equals(id))
            {
                ByteArrayInputStream validatorStream = new ByteArrayInputStream("<Validation><regexp>^([\\w\\-\\.])+@([\\w\\-\\.])+\\.([a-zA-Z])+$</regexp></Validation>".getBytes());
                Configuration validatorConf = new DefaultConfigurationBuilder().build(validatorStream);
                DefaultValidator validator = new DefaultValidator();
                validator.enableLogging(getLogger());
                validator.setPluginInfo(BASE_PLUGIN_NAME, null);
                ...
                parameter = new JdbcParameter(BASE_PLUGIN_NAME, id, column, "PLUGINS_CORE_USERS_JDBC_FIELD_EMAIL_LABEL", "PLUGINS_CORE_USERS_JDBC_FIELD_EMAIL_DESCRIPTION",
                        ParameterHelper.TYPE.STRING, null, null, validator);
            }
            else
            {
