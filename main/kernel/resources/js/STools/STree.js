...
STree.aToLink = function(link, _document, tree, i)
{
	...
	link.tree = tree
	link.href = "#"
	link.oldonclick = link.onclick && link.onclick != STree.normalClick ? link.onclick : function () {} ;
...
STree.openClose = function (event)
{
	...
	if (elt.className == "opened_intermediate")
    {
        ...
        if (tree.stree.listener && tree.stree.listener.onCloseNode)
            tree.stree.listener.onCloseNode(tree.stree, elt);
    }
	else if (elt.className == "closed_intermediate")
    {
		elt.className = "opened_intermediate";
        elt.style.display = "";
        if (tree.stree.listener && tree.stree.listener.onOpenNode)
            tree.stree.listener.onOpenNode(tree.stree, elt);
    }
...
STree.prototype.insertNode = function(elt, img, label, action)
{
    ...
    if (parentLi.className.indexOf("closed") == 0 && tree.stree.listener && tree.stree.listener.onOpenNode)
    {
        tree.stree.listener.onOpenNode(tree.stree, parentLi);
    }
