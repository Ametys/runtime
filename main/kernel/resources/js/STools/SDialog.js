...
SDialog.prototype.paint = function() {
	...
	this.ui.iframe.style.zIndex = "+5";
	this.ui.iframe.style.backgroundColor = "#ece9d8";
	this.ui.iframe.frameBorder = "no";

	this.ui.table.parentNode.insertBefore(this.ui.iframe, this.ui.table);
	...
	_document.open();
	_document.write("<html><body style='padding: 0px; margin: 0px'></body></html>")
	_document.close();
	...
	if (!SDialog.initialization)
	{
		...
		SDialog.registerOnMouseMoveEvent();
...
{	
	if (SDialog._sprotect.style.display == 'block')
		return;
	
	SDialog.currentOpen.push(this);
	SDialog._sprotect.style.display = 'block';
...
{	
	...
	{
		SDialog._sprotect.style.display = 'none';
		SDialog._sprotect.style.left = '-100';
		SDialog._sprotect.style.top = '-100';
	}
...
SDialog.attachEvent = function(element, event, handler)
{  
	if (!STools.is_ie)   
	{ 
...
SDialog.detachEvent = function(element, event, handler)
{
	if (!STools.is_ie)
	{
...
SDialog.registerOnResize = function () {
	SDialog.attachEvent(document.body, "resize", SDialog._registerOnResize);
	if (!STools.is_ie )
...
SDialog.registerOnMouseMoveEvent = function()
{
	if (!STools.is_ie)
	...
	SDialog.mouseX = 0;
	SDialog.mouseY = 0;
	SDialog.attachEvent(document, "mousemove", 
		function (e) { // catches and processes the mousemove event
                SDialog.moveOutside(e, document)
		});
}

SDialog.registerOnMouseMoveEventForDocument = function (_document)
{
	if (!STools.is_ie)
		_document.captureEvents(Event.MOUSEMOVE) // Defines what events to capture for Navigator

	_document.olderonmousemove = _document.onmousemove;
	_document.onmousemove = function (e) { // catches and processes the mousemove event
                SDialog.moveOutside(e, this)
	};
}
...
SDialog.move = function(top, left, draggingElement)
{
	...
	if (STools.is_ie)
	{
		maxLeft = document.body.scrollLeft + document.body.offsetWidth - draggingElement.offsetWidth - 20;
		maxTop = document.body.scrollTop + document.body.offsetHeight - draggingElement.offsetHeight - 20;
	}
	else
	{
...
SDialog.moveOutside = function(e, _document)
{
	...
	if (SDialog.currentOpen.length > 0)
	{
		SDialog._sprotect.style.left = SDialog.mouseX;
		SDialog._sprotect.style.top = SDialog.mouseY;
	}
	if (SDialog.dragging)
	{
...
SDialog.createProtection = function()
{
	var table = document.createElement("TABLE");
		...
		table.dialog = this;
		table.onclick = SDialog.clickOutside;
	...
	var col = document.createElement("TD");
		col.dialog = this;
		col.onclick = table.onclick;
		col.onmouseup = table.onmouseup;
		col.appendChild(document.createTextNode(" "));
		
	row.appendChild(col);
	
	SDialog._sprotect = table;
	document.body.appendChild(SDialog._sprotect);
}
