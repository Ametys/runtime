...
Ext.define('Ametys.form.ConfigurableFormPanel', {
	...
	tabErrorFieldsTpl: [
		...
		'<tpl if="errors && errors.length">',
			'<tpl if="errors.length == 1">',
				"<span class=\"ametys-tab-tooltip-error-label\"><i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_ERROR_FIELD'/></span>",
			'</tpl>',
			'<tpl if="errors.length != 1">',
				"<span class=\"ametys-tab-tooltip-error-label\">{errors.length}<i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_ERROR_FIELDS'/></span>",
			'</tpl>',
		    '<ul class="error"><tpl for="errors"><li>{.}</li></tpl></ul>',
		'</tpl>',
		'<tpl if="warns && warns.length">',
			'<tpl if="warns.length == 1">',
				"<span class=\"ametys-tab-tooltip-warn-label\"><i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_WARN_FIELD'/></span>",
			'</tpl>',
			'<tpl if="warns.length != 1">',
				"<span class=\"ametys-tab-tooltip-warn-label\">{warns.length}<i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_WARN_FIELDS'/></span>",
			'</tpl>',
		    '<ul class="warn"><tpl for="warns"><li>{.}</li></tpl></ul>',
		'</tpl>',
		'<tpl if="comments && comments.length">',
			'<tpl if="comments.length == 1">',
				"<span class=\"ametys-tab-tooltip-comment-label\"><i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_COMMENT_FIELD'/></span>",
			'</tpl>',
			'<tpl if="comments.length != 1">',
				"<span class=\"ametys-tab-tooltip-comment-label\">{comments.length}<i18n:text i18n:key='CONFIGURABLE_FORM_TAB_TPL_COMMENT_FIELDS'/></span>",
			'</tpl>',
	...
	* @private
	...
	{
		tabpanel.mask("<i18n:text i18n:key='CONFIGURABLE_FORM_INLINETAB_WAIT_MSG'/>");
	...
	_doExpandOrCollapseAllInlineTab : function (tabpanel, btn, collapse)
	{
		...
		finally 
		{
			...
			btn.setText(collapse ? "<i18n:text i18n:key='CONFIGURABLE_FORM_INLINETAB_EXPAND_ALL'/>" : "<i18n:text i18n:key='CONFIGURABLE_FORM_INLINETAB_COLLAPSE_ALL'/>");
		}
	},
			...
			tabPanel = Ext.create('Ext.panel.Panel', {
				...
				dockedItems: [{
					...
					items:[
					    '->',
					    {
						    text: "<i18n:text i18n:key='CONFIGURABLE_FORM_INLINETAB_COLLAPSE_ALL'/>",
						    enableToggle: true,
	...
	_createInputField: function (config)
	{
	    ...
	    Ext.applyIf (fieldCfg, {
			...
			regexText: config.regexText || config.invalidText || "<i18n:text i18n:key='CONFIGURABLE_FORM_INVALID_REGEXP'/>" + config.regexp,
		...
		if (config.type.toLowerCase() == Ametys.runtime.form.WidgetManager.TYPE_RICH_TEXT)
		{
			widgetCfg.listeners['editorhtmlnodeselected'] = { fn: function (field, node) { this.fireEvent ('htmlnodeselected', field, node)}, scope: this};
		}
		
		return Ametys.runtime.form.WidgetManager.getWidget (config.widget, config.type.toLowerCase(), widgetCfg);
	},
	...
	_configureJSON: function (data, prefix, ct, offset, roffset)
	{
			...
			if (tabs.length > 0)
			{
				...
				{
					var tab = this._addTabItem (tabPanel, tabs[i].label || "<i18n:text i18n:key='CONFIGURABLE_FORM_TAB_UNTITLED'/>");
	...
	_configureXML: function (metadataNode, prefix, ct, offset, roffset)
	{
			...
			if (tabs.length > 0)
			{
				...
				for (var i=0; i < tabs.length; i++)
				{
					var tab = this._addTabItem (tabPanel, Ext.dom.Query.selectValue('> label', tabs[i], "<i18n:text i18n:key='CONFIGURABLE_FORM_TAB_UNTITLED'/>"));
					// Transmit offset + 5 (padding) + 1 (border) + 11 (margin + border) if we are in a nested composite.
					var finalOffset = offset 
									+ Ametys.form.ConfigurableFormPanel.PADDING_TAB 
									+ 1; 
	...
	_onCommentClick: function(evt, el, args)
	{
		...
		if (!this._commentsBox)
		{
			this._commentsBox = Ext.create('Ametys.window.DialogBox', {
				title: "<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_DIALOGBOX_TITLE'/>",
				...
				buttons : [{
					itemId: 'button-ok',
					text: "<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_BTN_OK'/>",
					handler: Ext.emptyFn
				}, {
					text: "<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_BTN_DELETE'/>",
					handler: Ext.emptyFn,
					hidden: true
				}, {
					text: "<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_BTN_CANCEL'/>",
		...
		this._commentsBox.setTitle ("<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_DIALOGBOX_TITLE'/>" + ' - ' + fdLabel);
	...
	_onCommentClickDelete: function(field)
	{
		var me = this;
		Ametys.Msg.confirm(
			"<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_DELETE_CONFIRM_TITLE'/>",
			"<i18n:text i18n:key='CONFIGURABLE_FORM_COMMENTS_DELETE_CONFIRM_MSG'/>",
			function (btn) {
				if (btn == 'yes')
				{
					field.removeComments();
