...
public final class ConfigManager implements Contextualizable, Serviceable, Initializable
{
    ...
    public void validate()
    {
        ...
        for (String id : _usedParamsName.keySet())
        {
            ...
            if (_params.get(id) == null)
            {
                ...
                if (info == null)
                {
                    String message = "The parameter '" + id + "' is not used but not declared.";
    ...
    private ConfigParameter _configureParameter(ConfigParameterInfo info)
    {
        ...
        try
        {
            labelKey = _configureLabelKey(conf);
            descriptionKey = _configureDescriptionKey(conf);
        ...
        return new ConfigParameter(id, info.getPluginName(), labelKey, descriptionKey, type, defaultValue, widget, displayCategory, displayGroup, order, enumeratorConf != null, validatorConf != null);
    }
    ...
    private void _saxParameters(Map<String, Map<String, List<ConfigParameter>>> categories, ContentHandler handler) throws SAXException
    {
        ...
        while (catIt.hasNext())
        {
            ...
            Map category = categories.get(categoryKey);

            AttributesImpl catAttrs = new AttributesImpl();
            catAttrs.addAttribute("", "label", "label", "CDATA", categoryKey.substring(categoryKey.indexOf(PluginsManager.FEATURE_ID_SEPARATOR) + PluginsManager.FEATURE_ID_SEPARATOR.length()));
            catAttrs.addAttribute("", "catalogue", "catalogue", "CDATA", categoryKey.substring(0, categoryKey.indexOf(PluginsManager.FEATURE_ID_SEPARATOR)));
            XMLUtils.startElement(handler, "category", catAttrs);

            XMLUtils.startElement(handler, "groups");

            Iterator groupIt = category.keySet().iterator();
            while (groupIt.hasNext())
            {
                String groupKey = (String) groupIt.next();
                List group = (List) category.get(groupKey);

                AttributesImpl gpAttrs = new AttributesImpl();
                gpAttrs.addAttribute("", "label", "label", "CDATA", groupKey.substring(groupKey.indexOf(PluginsManager.FEATURE_ID_SEPARATOR) + PluginsManager.FEATURE_ID_SEPARATOR.length()));
                gpAttrs.addAttribute("", "catalogue", "catalogue", "CDATA", groupKey.substring(0, groupKey.indexOf(PluginsManager.FEATURE_ID_SEPARATOR)));
                XMLUtils.startElement(handler, "group", gpAttrs);

                Iterator gIt = group.iterator();
                while (gIt.hasNext())
                {
                    ConfigParameter param = (ConfigParameter) gIt.next();
                    _saxParameter(handler, param, untypedValues);
                }
    ...
    private void _saxParameter(ContentHandler handler, ConfigParameter param, Map<String, String> untypedValues) throws SAXException
    {
        ...
        XMLUtils.createElement(handler, "label", param.getLabelKey());
        XMLUtils.createElement(handler, "description", param.getDescriptionKey());
        ...
        if (param.hasEnumerator())
        {
            ...
            for (EnumeratorValue enumeratorValue : values)
            {
                ...
                // Produit l'option
                XMLUtils.startElement(handler, "option");
                XMLUtils.createElement(handler, "value", valueAsString);
                label.toSAX(handler, "label");
     ...
     */
    public void save(Map untypedValues, String fileName) throws Exception
    {
        // Type values
        ...
        {
            String id = ids[i];
            String untypedValue = (String) untypedValues.get(id);
            
            Object typedValue = ParameterHelper.castValue(untypedValue, _params.get(id).getType());
            if (typedValue == null && _params.get(id).getType() == ParameterHelper.TYPE.PASSWORD && Config.getInstance() != null)
            {
     ...
     * @throws SAXException if an error occured
     */
    private void _toSAX(ContentHandler handler, Map<String, Object> typedValues) throws SAXException
    {
        handler.startDocument();
        XMLUtils.startElement(handler, "config");

        Iterator<String> it = typedValues.keySet().iterator();
        while (it.hasNext())
        {
            String key = it.next();

            Object typedValue = typedValues.get(key);
            String untypedValue = ParameterHelper.valueToString(typedValue);
            if (untypedValue == null)
            {
                untypedValue = "";
            ...
            XMLUtils.createElement(handler, key, untypedValue);
        }
    ...
    class ConfigParameter
    {
        ...
        ConfigParameter(String id, String pluginName, String labelKey, String descriptionKey, ParameterHelper.TYPE type, Object defaultValue, String widget, String displayCategory, String displayGroup, int order, boolean hasEnumerator, boolean hasValidator)
        {
            _id = id;
            _pluginName = pluginName;
            _labelKey = labelKey;
