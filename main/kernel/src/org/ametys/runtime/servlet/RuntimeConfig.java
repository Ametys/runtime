...
public final class RuntimeConfig
{
    ...
    private static void _configureApplication(Configuration config)
    {
        ...
        if (strDate != null && !"".equals(strDate))
        {
            try
            {
                __config._buildDate = new SimpleDateFormat("yyyyMMdd'T'HHmm").parse(strDate);
            }
            catch (ParseException e)
            {
                __LOGGER.warn("Unable to parse date '" + strDate + "' with format \"yyyyMMdd'T'HHmm\". It will be ignored.");
            }
        }
    }
