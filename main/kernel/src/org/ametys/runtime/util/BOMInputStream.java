 ...
 * </pre>
 * 
 * @see org.ametys.runtime.util.ByteOrderMark
...
public class BOMInputStream extends ProxyInputStream
{
     ...
     */
    private static final Comparator<ByteOrderMark> _BYTE_ORDER_MARK_LENGTH_COMPARATOR = new Comparator<ByteOrderMark>()
    {
     ...
     */
    private final List<ByteOrderMark> _boms;

    private ByteOrderMark _byteOrderMark;

    private int[] _firstBytes;

    private int _fbLength;

    private int _fbIndex;

    private int _markFbIndex;

    private boolean _markedAtStart;

    /**
     * Constructs a new BOM InputStream that excludes a
     * {@link ByteOrderMark#UTF_8} BOM.
     * 
     ...
     * {@link ByteOrderMark#UTF_8} and optionally includes it.
     * 
     * @param delegate the InputStream to delegate to
     ...
     * 
     * @param delegate the InputStream to delegate to
     * @param include true to include the specified BOMs or false to exclude
     *            them
    ...
    public BOMInputStream(InputStream delegate, boolean include, ByteOrderMark... boms)
    {
        ...
        Arrays.sort(boms, _BYTE_ORDER_MARK_LENGTH_COMPARATOR);
        this._boms = Arrays.asList(boms);
    }
    ...
    {
        if (!_boms.contains(bom))
        {
            throw new IllegalArgumentException("Stream not configure to detect " + bom);
        }
        return _byteOrderMark != null && getBOM().equals(bom);
    }
    ...
    public ByteOrderMark getBOM() throws IOException
    {
        if (_firstBytes == null)
        {
            _fbLength = 0;
            // BOMs are sorted from longest to shortest
            final int maxBomSize = _boms.get(0).length();
            _firstBytes = new int[maxBomSize];
            // Read first maxBomSize bytes
            for (int i = 0; i < _firstBytes.length; i++)
            {
                _firstBytes[i] = in.read();
                _fbLength++;
                if (_firstBytes[i] < 0)
                {
                    break;
            ...
            if (_byteOrderMark != null)
            {
                if (!_include)
                {
                    if (_byteOrderMark.length() < _firstBytes.length)
                    {
                        _fbIndex = _byteOrderMark.length();
                    }
                    else
                    {
                        _fbLength = 0;
    ...
    {
        getBOM();
        return _byteOrderMark == null ? null : _byteOrderMark.getCharsetName();
    }
    ...
    {
        getBOM();
        return _fbIndex < _fbLength ? _firstBytes[_fbIndex++] : -1;
    }
