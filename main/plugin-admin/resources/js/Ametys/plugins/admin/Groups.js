...
Ext.define('Ametys.plugins.admin.Groups', {
	...
	createPanel: function ()
	{
		...
		this._listViewGp = new Ext.grid.Panel({
			...
			title : "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_LABEL'/>",
			hideHeaders: true,
			border: false,
			autoScroll: true,
		...
		this._listViewU = new Ext.grid.Panel({
			...
			title : "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_COLUMN'/>",
			hideHeaders: true,
			multiSelect: true,
	...
	_selectGroup: function (me, record, index, eOpts)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_LIST_ERROR'/>", result, "Ametys.plugins.admin.Groups._selectGroup"))
	    {
	...
	_editGroupLabel: function (editor, e, eOpts)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_RENAME_ERROR'/>", result, "Ametys.plugins.admin.Groups._editGroupLabel"))
	    {
	       // nothing
	    }
		else
	...
	save: function (group, objects)
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_MODIFY_ERROR'/>", result, "Ametys.plugins.admin.Groups.save"))
	    {
	...
	{
		this._groupsActions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_CATEGORY'/>"});

		if (this._modifiable)
		{
			// New group
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_CREATE'/>",
					 null,
					 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/add_group.png', 
					 Ext.bind(this.add, this));
			
			// Rename goup
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_RENAME'/>",
					 null,
					 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/rename.png', 
					 Ext.bind(this.rename, this));
			
			// Delete goup
			this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_DELETE'/>",
					 null,
		...
		// Quit
		this._groupsActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_QUIT'/>",
				 null,
	...
	_drawUsersActionsPanel: function ()
	{
		...
		this._usersActions = new Ametys.admin.rightpanel.ActionPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_USERS_HANDLE_CATEGORY'/>"});

		// Add user
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_ADDUSER'/>",
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/add_user.png', 
				 Ext.bind(this.addUser, this));
		
		// Delete user
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_DELETEUSER'/>", 
				 null,
				 Ametys.getPluginResourcesPrefix(this.pluginName) + '/img/groups/delete.png', 
				 Ext.bind(this.deleteUsers, this));
		
		// Validate modification
		this._usersActions.addAction("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_VALIDATE'/>", 
				 null,
	...
	{
		var helpPanel = new Ametys.admin.rightpanel.TextPanel({title: "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HELP_CATEGORY'/>"});
		helpPanel.addText("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_HELP_HINT'/>");
	...
	add: function ()
	{
		...
		var result = Ametys.data.ServerComm.send({
			...
			url: "/groups/create", 
			parameters: { name: "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_NEWGROUP'/>" }, 
			priority: Ametys.data.ServerComm.PRIORITY_SYNCHRONOUS, 
			callback: null, 
			responseType: null
		});
		
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_NEW_ERROR'/>", result, "Ametys.plugins.admin.Groups.add"))
	    {
	       return false;
	    }
		...
		var newEntry = Ext.create('Ametys.plugins.admin.Groups.Group', {
			'name': "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_NEWGROUP'/>",
			'id': id
		}); 
	...
	remove: function () 
	{
		Ametys.Msg.confirm ("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_HANDLE_DELETE'/>", 
						 "<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_DELETE_CONFIRM'/>", 
	...
	doRemove: function (answer)
	{
		if (answer == 'yes')
		{
	    	...
	    	if (200 == Ext.Ajax.request({url: Ametys.getPluginDirectPrefix(this.pluginName) + "/groups/delete", params: "id=" + elt.get('id'), async: false}).status)
			{
	...
	checkBeforeQuit: function()
    {
        if (this._currentGroup != null && this._hasChanges)
        {
            if (confirm("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_MODIFY_CONFIRM'/>"))
            {
                this.save();
            }
        }
