...
Ext.define('Ametys.plugins.admin.datasource.EditSQLDataSourceHelper', {
	...
	/**
	 * @private
	 * @property {Boolean} _initialized True if the dialog box creation process
	 *           is finished
	 */
	/**
	 * @private
	...
	/**
	 * @private
	 * @property {Object} _driverTemplates the mapping of driver value with
	 ...
	 * 
	 * @param {Function}
	 *            [callback] a callback function to invoke after the data source
	 *            was created.
	 */
	 ...
	 * 
	 * @param {String}
	 *            id the id of the selected data source
	 * @param {Function}
	 *            [callback] a callback function to invoke after the data source
	 *            was edited.
	 */
	...
	/**
	 * @private Show dialog box for SQL data source edition
	 * @param {String}
	 *            id the id of the selected data source in 'edit' mode, null in
	 *            'new' mode
	 */
	_open : function(id) {
		var me = this;
	...
	/**
	 * @private Create the dialog box if it is not already the case
	 * @param {Function}
	 *            callback function invoked when the dialog box's drawing is
	 *            finished
	 */
	_createDialogBox : function(callback) {
		if (!this._initialized) {
			this._form = Ext.create('Ametys.form.ConfigurableFormPanel', {
						testURL : Ametys.getPluginDirectPrefix('admin')
								+ '/datasource/test'
					});

			this._box = Ext.create('Ametys.window.DialogBox', {
				title : this._mode == 'new'
						? "<i18n:text i18n:key='PLUGINS_ADMIN_DATASOURCES_DIALOG_SQL_ADD_TITLE'/>"
						: "<i18n:text i18n:key='PLUGINS_ADMIN_DATASOURCES_DIALOG_SQL_EDIT_TITLE'/>",
				iconCls : "flaticon-data110",
			...
			this._box
					.setTitle(this._mode == 'new'
							? "<i18n:text i18n:key='PLUGINS_ADMIN_DATASOURCES_DIALOG_SQL_ADD_TITLE'/>"
							: "<i18n:text i18n:key='PLUGINS_ADMIN_DATASOURCES_DIALOG_SQL_EDIT_TITLE'/>");
			this._box.setIconCls("flaticon-data110 "
					+ (this._mode == 'new'
							? "decorator-flaticon-add64"
							: "decorator-flaticon-edit45"));
			callback(true);
		}
	},

	/**
	 * @private Configure the {@link Ametys.form.ConfigurableFormPanel}
	 * @param {Function}
	 *            callback function invoked when the form configuration is
	 *            finished
	 */
	...
	/**
	 * @private Callback for the form configuration process. Configures the
	 *          {@link Ametys.form.ConfigurableFormPanel} of the dialog box
	 * @param {Object}
	 *            response the server's response
	 * @param {Array}
	 *            response.databaseTypes the array of database types
	 * @param {String}
	 *            response.databaseTypes.label the label of the current database
	 *            type
	 * @param {String}
	 *            response.databaseTypes.value the value of the current database
	 *            type
	 * @param {String}
	 *            response.databaseTypes.template the template associated to the
	...
	_getSQLDatabaseTypesCb : function(response, args) {
		...
		var driverEnumeration = []; // The list of label-value pairs that will
									// be displayed in the form
		var templateMapping = {}; // The mapping of driver value and
									// associated template

		Ext.Array.each(databaseTypes, function(databaseType) {
					driverEnumeration.push({
								label : databaseType.label,
	...
	/**
	 * @private Get the form configuration
	 * @param {Object[]}
	 *            driverEnumeration the drivers' enumeration
	...
	/**
	 * @private Initialize the fields of the form
	 * @param {String}
	 *            id the id of the data source if in 'edit' mode, null in 'new'
	 *            mode
	 */
	_initForm : function(id) {
		var form = this._form.getForm();
		if (!this._initialized) {
			form.findField('driver').on('change',
					Ext.bind(this._onDriverChange, this));
		} else {
			// Reset the parameter checker status to 'not tested' and reset the
			// warnings
			this._form.reset();
		}
		...
		if (id == null) {
			...
			var me = this;
			Ametys.plugins.core.datasource.DataSourceDAO.getDataSource(["SQL",
	...
	/**
	 * @private Check if the data source is valid
	 */
	_ok : function() {
		...
		Ext
				.getBody()
				.mask("<i18n:text i18n:key='PLUGINS_CORE_UI_LOADMASK_DEFAULT_MESSAGE' i18n:catalogue='plugin.core-ui'/>");
		paramCheckersDAO.check(paramCheckersDAO._paramCheckers, true, Ext.bind(
						function(success) {
							Ext.getBody().unmask();
							if (success) {
								this._okCb();
	...
	/**
	 * @private Submit the form's values
	 */
	_okCb : function() {
		var values = this._form.getValues();
		values['private'] = this._form.getForm().findField('private')
				.getValue();

		if (this._mode == 'new') {
			Ametys.plugins.core.datasource.DataSourceDAO.addDataSource(["SQL",
							values], this._addOrEditDataSourceCb, {
						scope : this,
						waitMessage : {
							target : this._box
						}
			...
			Ametys.plugins.core.datasource.DataSourceDAO.editDataSource(["SQL",
							values], this._addOrEditDataSourceCb, {
						scope : this,
						waitMessage : {
							target : this._box
	...
	/**
	 * @private Function invoked when the value of the driver field changes
	 * @param {Ametys.form.widget.ComboBox}
	 *            combo the combobox
	 * @param {String}
