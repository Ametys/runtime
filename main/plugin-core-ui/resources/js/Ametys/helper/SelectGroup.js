...
Ext.define('Ametys.helper.SelectGroup', {
	...
	delayed_initialize: function ()
	{
		...
		var model = Ext.define('Ametys.helper.SelectGroup.Groups', {
		...
		var store = Ext.create('Ext.data.Store', {
			model: 'Ametys.helper.SelectGroup.Groups',
	        data: { groups: []},
	        proxy: {
	        	type: 'memory',
	        	reader: {
	...
	load: function ()
	{
	    ...
	    if (Ametys.data.ServerComm.handleBadResponse("<i18n:text i18n:key='PLUGINS_CORE_UI_GROUPS_SELECTGROUP_DIALOG_ERROR_LISTING'/>", result, "Ametys.helper.SelectGroup.load"))
	    {
	       return;
	    }
		...
		for (var i=0; i < groups.length; i++)
		{
			...
			var group = Ext.create('Ametys.helper.SelectGroup.Groups', {
