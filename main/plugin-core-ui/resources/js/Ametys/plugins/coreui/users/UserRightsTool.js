 ...
 * @private
 */
Ext.define('Ametys.plugins.coreui.users.UserRightsTool', {
	...
	_toolTitleTpl: new Ext.Template(
			"{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_TITLE}}",
			" {fullname}"
	),
	...
	_toolDescriptionTpl: new Ext.Template(
			"{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_TOOLTIPTEXT}}",
			" {fullname} ({login})"
	),
	 ...
	 */
	_contextByProfileHintTpl: new Ext.Template("{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_CONTEXT_BY_PROFILE_HINT_1}}",
			" {fullname} ({login})",
			"{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_CONTEXT_BY_PROFILE_HINT_2}}"
	),
	 ...
	 */
	_rightsByContextHintTpl: new Ext.Template("{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_RIGHTS_BY_CONTEXT_HINT_1}}",
			" {fullname} ({login})",
			"{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_RIGHTS_BY_CONTEXT_HINT_2}}"
	),
	...
	refresh: function (manual)
	{
        ...
        this._population = userTarget.getParameters().population;
		
		Ametys.data.ServerComm.callMethod({
			role: "org.ametys.plugins.core.right.UsersClientInteraction",
			methodName: "getUserRights",
			parameters: [this._login, this._population],
			callback: {
 				scope: this,
			...
			waitMessage: true,
			errorMessage: {
				msg: "{{i18n PLUGINS_CORE_UI_TOOL_USER_RIGHTS_ERROR}}",
	...
	_drawContextByProfiles: function (user, profiles)
	{
		...
		Ext.Array.each (profiles, function (profile) {
			...
			var profilePanel = Ext.create ('Ext.Panel', {
				...
				title: '<img src="' + Ametys.getPluginResourcesPrefix('core-ui') + '/img/profiles/profile_16.png' + '" style="float:left; margin-right:5px;"' + '"/>' + profile.label,
				border: false,
				ui: 'light',
				cls: 'a-panel-text',
				header: {
